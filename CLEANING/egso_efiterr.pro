...
; NAME              : egso_efiterr
;
; PURPOSE           : calculating the absolute error of the fit
;
; EXPLANATION       : calculating the absolute error of the fit
;
;
; CATEGORY          : EGSO_WP5, Fitting an Ellipse
;
; CALLING SEQUENCE  : egso_efiterr, binary, xc, yc, vector, binary2, stdev, stdevGeo, abs_err, geoErr, [/display]
;
;
; INPUT             : binary     binary image with the data
;                     xc, yc     coordinate centre
;                     vector     ellipse parameters
;
; OUTPUT            : binary2    the cleaned binary with the outliers removed
;                     stdev      fits standard deviation (algebraic)
;                     stdevGeo   fits standard deviation (geometric)
;                     algerr     fits algebraic error
;
;
; KEYWORDS          : display     if set display results (Geometric Error, Standard Deviation (pixels), mean, ...)
;
; PROCEDURE         :
;
; CALLS             : stddev, moment
;
; EXAMPLE           : egso_efiterr, binary, xc, yc, vector, binary2, stdev, stdevGeo, algerr, geoErr
;
; SIDE EFFECTS      : None.
;
...
pro egso_efiterr, binary, xc, yc, vector, binary2, stdev, stdevGeo, abs_err, geoErr, display=display
